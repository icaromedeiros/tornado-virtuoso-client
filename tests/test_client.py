...
from tornado_virtuoso_client.client import VirtuosoConnection
...
class TriplestoreTestCase(AsyncTestCase):
    ...
    @patch("tornado_virtuoso_client.settings",
           SPARQL_ENDPOINT=EndpointConfig.URL,
           )
    def test_query_ok(self, settings):
        virtuoso_connection = VirtuosoConnection(self.io_loop)
    ...
    @patch("tornado_virtuoso_client.settings", SPARQL_ENDPOINT=EndpointConfig.AUTHENTICATED_URL)
